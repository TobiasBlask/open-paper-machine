...
\begin{lstlisting}[caption={Directory structure of the Open Academic Paper Machine plugin (v5.5.0). The system comprises 14 skill engines, 15 slash commands, 1 pipeline agent, 2 utility scripts, and 1 \LaTeX\ template.},label={lst:tree},float=t]
open-paper-machine/
+-- .claude-plugin/
...
\midrule
\multicolumn{4}{l}{\emph{Extended-capability engines (v5.5.0)}} \\
...
\caption{The eight-phase pipeline of the Open Academic Paper Machine (v5.5.0) with human quality-gate checkpoints. Each phase is executed by specialized skill engines and produces persistent artifacts. Since v5.5.0, the pipeline agent suggests extended-capability tools at relevant stages (screening after Phase~1, positioning after Phase~2, and the post-production suite after Phase~6). Phases~7 (verification) and~8 (revision) are optional and repeatable.}
\label{fig:pipeline}
\end{figure}
